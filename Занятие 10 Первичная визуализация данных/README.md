# Первичная визуализация данных / Primary Data Visualization

Раздел соответствует теме **«Первичная визуализация данных»**. Английское имя используется как технический путь GitHub.

## Материалы / Materials
- [Integrated project visualization notebook](../../add-classical-data-analytics/01_foundations/06_data_integration/integrated_project/notebooks_student/05_basic_visualization.ipynb)
**Ссылка на пример:** [Открыть 05_basic_visualization](https://github.com/TheRisingWolf/DataAnalysis-2026-part3/blob/main/Занятие%2010%20Первичная%20визуализация%20данных/Пример%201/06_data_integration/integrated_project/notebooks_student/05_basic_visualization.ipynb)

- [Advanced support analytics EDA notebook](../../add-classical-data-analytics/shared/cases/support_analytics/notebooks/student/01_data_quality_eda_student.ipynb)

**Ссылка на пример:** [Advanced support analytics EDA notebook](https://github.com/q10sn1k/DA_MAY_2026/blob/main/add-classical-data-analytics/shared/cases/support_analytics/notebooks/student/01_data_quality_eda_student.ipynb)

## Как использовать
Перед корректным запуском начального ноутбука "05_basic_visualization.ipynb" по теме "Первичная визуализация данных" запустите ноутбук "03_data_integration.ipynb" от самого начала до конца (от самой первой ячейки с кодом до самой последней ячейки с кодом включительно) для создания необходимых подготовленных данных. 

Учебное содержимое может оставаться на русском языке, а технические имена стандартизированы на английском. Преподавательские решения, внутренние QA-файлы и лишние дубли в комплект слушателя не включены.

[← Общий навигатор](../../README.md)
